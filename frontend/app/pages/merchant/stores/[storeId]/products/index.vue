<template>
	...
	<main class="md:ml-64 pt-20 min-h-screen bg-slate-50">
		<div class="max-w-[1280px] mx-auto px-6 py-8">
			<!-- Header Section -->
			<div class="flex flex-col md:flex-row md:items-end justify-between gap-4 mb-8">
				<div>
					<nav class="flex text-xs text-slate-500 mb-2 gap-2">
						<span>Stores</span> <span>/</span> <span>Main Store</span> <span>/</span> <span
							class="text-slate-900 font-bold">Products</span>
					</nav>
					<h1 class="font-bold tracking-tight text-4xl leading-tight text-slate-900">Products</h1>
					<p class="text-slate-600 font-normal mt-1">Manage your store's inventory, stock levels,
						and product visibility.</p>
				</div>
				<button
					class="bg-slate-800 text-white px-6 py-3 rounded-lg font-bold flex items-center gap-2 active:scale-95 transition-all shadow-md">
			...
			<div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
				<div class="bg-white border border-slate-400 p-4 rounded-xl">
					<p class="text-xs text-slate-600 uppercase tracking-wider mb-1">Total Products</p>
					<p class="text-3xl leading-tight font-bold tracking-tight">1,284</p>
				</div>
				<div class="bg-white border border-slate-400 p-4 rounded-xl">
					<p class="text-xs text-slate-600 uppercase tracking-wider mb-1">Low Stock Items</p>
					<div class="flex items-center gap-2">
						<p class="text-3xl leading-tight font-bold tracking-tight text-red-600">12</p>
						<span class="material-symbols-outlined text-red-600" data-icon="warning">warning</span>
					</div>
				</div>
				<div class="bg-white border border-slate-400 p-4 rounded-xl">
					<p class="text-xs text-slate-600 uppercase tracking-wider mb-1">Out of Stock</p>
					<p class="text-3xl leading-tight font-bold tracking-tight">3</p>
				</div>
				<div class="bg-white border border-slate-400 p-4 rounded-xl">
					<p class="text-xs text-slate-600 uppercase tracking-wider mb-1">Drafts</p>
					<p class="text-3xl leading-tight font-bold tracking-tight text-slate-600">45</p>
				</div>
			</div>
			<!-- Filter Bar -->
			<div class="bg-white border border-slate-400 rounded-xl p-4 mb-6 shadow-sm">
				<div class="flex flex-col lg:flex-row gap-4">
					<div class="flex flex-wrap items-center gap-3">
						<div class="relative">
							<select
								class="appearance-none bg-slate-50 border-slate-400 border px-4 py-2 pr-10 rounded-md font-semibold text-slate-600 outline-none focus:ring-2 focus:ring-primary-container transition-all">
						...
						<button
							class="flex items-center gap-2 border border-slate-400 px-4 py-2 rounded-md font-semibold text-slate-600 hover:bg-slate-100 transition-colors">
			...
			<div class="bg-white border border-slate-400 rounded-xl overflow-hidden shadow-sm">
					...
					<table class="w-full text-left border-collapse">
						<thead>
							<tr class="bg-slate-50 border-b border-slate-400">
								<th class="p-4 w-10">
									<input
										class="rounded border-slate-400 text-slate-800 focus:ring-primary-container"
										type="checkbox" />
								</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">Product
								</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">SKU</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">Category
								</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">Price
								</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">Stock
								</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">Status
								</th>
								<th class="p-4 text-sm text-slate-600 uppercase tracking-wider">
									Visibility</th>
						...
						<tbody class="divide-y divide-slate-100">
							...
							<tr class="hover:bg-slate-50 transition-colors group">
								...
								<td class="p-4 font-semibold text-slate-600">LXC-982-WHT</td>
								<td class="p-4">
									<span
								...
								<td class="p-4">
									<div class="flex items-center gap-2">
										<div class="w-24 bg-slate-100 h-1.5 rounded-full overflow-hidden">
											<div class="bg-slate-900 h-full" style="width: 85%"></div>
										</div>
							...
							<!-- Product Row 2 (Low Stock Warning) -->
							<tr class="hover:bg-slate-50 transition-colors group">
								<td class="p-4">
									<input
										class="rounded border-slate-400 text-slate-800 focus:ring-primary-container"
								...
								<td class="p-4 font-semibold text-slate-600">RUN-551-RED</td>
								<td class="p-4">
									<span
								...
								<td class="p-4">
									<div class="flex items-center gap-2">
										<div class="w-24 bg-slate-100 h-1.5 rounded-full overflow-hidden">
											<div class="bg-red-600 h-full" style="width: 8%"></div>
										</div>
										<span class="text-xs font-bold text-red-600">8</span>
										<span class="material-symbols-outlined text-red-600 text-xs"
							...
							<tr class="hover:bg-slate-50 transition-colors group">
								...
								<td class="p-4 font-semibold text-slate-600">SNC-001-BLK</td>
								<td class="p-4">
									<span
										class="bg-slate-200 text-slate-800 px-2 py-1 rounded-full text-xs font-bold uppercase tracking-tight">Electronics</span>
								</td>
								...
								<td class="p-4">
									<div class="flex items-center gap-2">
										<div class="w-24 bg-slate-100 h-1.5 rounded-full overflow-hidden">
											<div class="bg-slate-900 h-full" style="width: 42%"></div>
										</div>
				...
				<!-- Pagination -->
				<div
					class="p-4 flex flex-col md:flex-row items-center justify-between border-t border-slate-100 bg-slate-50 gap-4">
					<p class="text-xs text-slate-600 font-medium">Showing 1 to 10 of 1,284 results</p>
					<div class="flex items-center gap-2">
						<button
							class="px-3 py-1 border border-slate-400 rounded-md text-xs font-bold bg-white hover:bg-slate-100 transition-colors disabled:opacity-50"
							disabled>Previous</button>
						<button
							class="px-3 py-1 border border-slate-800 rounded-md text-xs font-bold bg-slate-800 text-white">1</button>
						<button
							class="px-3 py-1 border border-slate-400 rounded-md text-xs font-bold bg-white hover:bg-slate-100 transition-colors">2</button>
						<button
							class="px-3 py-1 border border-slate-400 rounded-md text-xs font-bold bg-white hover:bg-slate-100 transition-colors">3</button>
						<span class="text-xs">...</span>
						<button
							class="px-3 py-1 border border-slate-400 rounded-md text-xs font-bold bg-white hover:bg-slate-100 transition-colors">129</button>
						<button
							class="px-3 py-1 border border-slate-400 rounded-md text-xs font-bold bg-white hover:bg-slate-100 transition-colors">Next</button>
					</div>
				</div>
			</div>
		</div>
